  ...
  <div class="page">
    ...
    <h1>Акции и скидки</h1>
      <div class="promotions">
          <div v-for="item in array" :key="item.promotionId" class="promotion__item">
              <img :src="item.imageGallery[0].imageUrl" alt="">
              <router-link :to="{ name: 'promotions__info', params: { promId: item.promotionId } }">
                {{getPromotionTitle(item)}}
              </router-link>
              <p class="date">{{ item.promotionsDate }}</p>
              <p class="description">{{ getDescription(item).substring(0,50) + "..."  }}</p>
          </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['promotions', 'loading']),
    array() {
      return this.setupPagination(this.promotions)
    }
  },
  beforeCreate() {
