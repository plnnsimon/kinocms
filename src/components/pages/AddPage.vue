<template>
  <div class="news-page">
    <div class="switcher">
      <p>Статус:</p>
      <label class="switch">
        <input v-model="page.checked" type="checkbox" />
    ...
    <div class="container">
      <div class="news-top-section">
        <div class="film-name">
          ...
          <input
          v-if="lang == 'ru'"
            v-model="page.ruTitle"
            type="text"
            :placeholder='$t("name")'
          />
          <input
          v-else
            v-model="page.uaTitle"
            type="text"
            :placeholder='$t("name")'
          />
        </div>
      ...
      <div class="main-picture">
        <p>{{ $t("mainImage") }}</p>
        <img :src="page.picture.imageUrl" alt="picture" />
        <input
          type="file"
          style="display: none"
          ref="fileInput"
      ...
      <div class="picture-gallery">
        ...
        <div class="images">
          <div
            class="gallery-image"
            v-for="(image, index) in page.imageGallery"
            :key="index"
          >
            <i
              class="fas fa-trash-alt"
              @click="removeGalleryImage(index)"
              v-if="page.imageGallery[index].imageUrl"
            ></i>
            <input
              :index="index"
              v-if="!page.imageGallery[index].imageUrl"
            ...
            <img
              v-if="page.imageGallery[index].imageUrl"
              :src="page.imageGallery[index].imageUrl"
            />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    onFileSelected(event) {
      ...
      this.page.picture.selectedFile = files[0].name;
      if (this.page.picture.selectedFile.indexOf(".") <= 0) {
        return alert("Please add a valid file");
      }
      const fileReader = new FileReader();
      fileReader.addEventListener("load", () => {
        this.page.picture.imageUrl = fileReader.result;
      });
      fileReader.readAsDataURL(files[0]);
      this.page.picture.image = files[0];
    },
    removeImage() {
      this.page.picture.imageUrl = "";
    },
    onGalleryImageSelected(index) {
      const fileReader = new FileReader();
      fileReader.addEventListener("load", () => {
        this.page.imageGallery[index].imageUrl = fileReader.result;
      });
      fileReader.readAsDataURL(this.$refs.galleryImageFile[0].files[0]);
    },
    removeGalleryImage(index) {
      this.page.imageGallery.splice(index, 1);
    },
    savePage() {
