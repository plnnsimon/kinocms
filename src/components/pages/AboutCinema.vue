<template>
  <div class="news-page">
    <div class="switcher">
      <p>Статус:</p>
      <label class="switch">
        <input v-model="aboutCinema.checked" type="checkbox" />
    ...
    <div class="container">
      ...
      <div class="main-picture">
        <p>{{ $t("mainImage") }}</p>
        <img :src="aboutCinema.picture.imageUrl" alt="picture" />
        <input
          type="file"
          style="display: none"
          ref="fileInput"
      ...
      <div class="picture-gallery">
        ...
        <div class="images">
          <div
            class="gallery-image"
            v-for="(image, index) in aboutCinema.imageGallery"
            :key="index"
          >
            <i
              class="fas fa-trash-alt"
              @click="removeGalleryImage(index)"
              v-if="aboutCinema.imageGallery[index].imageUrl"
            ></i>
            <input
              :index="index"
              v-if="!aboutCinema.imageGallery[index].imageUrl"
            ...
            <img
              v-if="aboutCinema.imageGallery[index].imageUrl"
              :src="aboutCinema.imageGallery[index].imageUrl"
            />
          </div>
        </div>
      ...
      <div class="seo">
        ...
        <form>
          ...
          <input
            v-model="aboutCinema.seo.keywords"
            type="text"
            id="keywords"
          ...
          <textarea
            v-model="aboutCinema.seo.description"
            type="text"
            id="description"
            :placeholder="$t('description')"
          ></textarea>
...
export default {
  ...
  methods: {
    ...
    addImage() {
      this.aboutCinema.imageGallery.push({
        imageUrl: "",
      });
    },
    languageActive() {},
    ...
    onFileSelected(event) {
      ...
      this.aboutCinema.picture.selectedFile = files[0].name;
      if (this.aboutCinema.picture.selectedFile.indexOf(".") <= 0) {
        return alert("Please add a valid file");
      }
      const fileReader = new FileReader();
      fileReader.addEventListener("load", () => {
        this.aboutCinema.picture.imageUrl = fileReader.result;
      });
      fileReader.readAsDataURL(files[0]);
      this.aboutCinema.picture.image = files[0];
    },
    removeImage() {
      this.aboutCinema.picture.imageUrl = "";
    },
    onGalleryImageSelected(index) {
      const fileReader = new FileReader();
      fileReader.addEventListener("load", () => {
        this.aboutCinema.imageGallery[index].imageUrl = fileReader.result;
      });
      fileReader.readAsDataURL(this.$refs.galleryImageFile[0].files[0]);
    },
    removeGalleryImage(index) {
      this.aboutCinema.imageGallery.splice(index, 1);
    },
    savePage() {
      this.$store.dispatch("addPage", this.aboutCinema);
    },
  },
};
</script>
